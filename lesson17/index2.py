...
class Window(tk.Tk):
    def __init__(self, **kwargs):
        ...
        self.listbox = tk.Listbox(
            bottomFrame, listvariable=choicesvar, width=12)
        self.listbox.pack(pady=20)
        bottomFrame.pack(expand=True, fill='x')
        self.listbox.bind("<<ListboxSelect>>", self.user_selected)

        resultFrame = tk.Frame(self)
        tk.Label(resultFrame, text="年度:").grid(
            column=0, row=0, sticky='E', pady=5)
        tk.Label(resultFrame, text="地區:").grid(
            column=0, row=1, sticky='E', pady=5)
        tk.Label(resultFrame, text="人口數:").grid(
            column=0, row=2, sticky='E', pady=5)
        tk.Label(resultFrame, text="土地面積:").grid(
            column=0, row=3, sticky='E', pady=5)
        tk.Label(resultFrame, text="人口密度:").grid(
            column=0, row=4, sticky='E', pady=5)
        self.yearVar = tk.StringVar()
        tk.Label(resultFrame, textvariable=self.yearVar).grid(
            column=1, row=0, sticky='W')
        self.cityVar = tk.StringVar()
        tk.Label(resultFrame, textvariable=self.cityVar).grid(
            column=1, row=1, sticky='W')
        self.population = tk.StringVar()
        tk.Label(resultFrame, textvariable=self.population).grid(
            column=1, row=2, sticky='W')
        self.areaVar = tk.StringVar()
        tk.Label(resultFrame, textvariable=self.areaVar).grid(
            column=1, row=3, sticky='W')
        self.densityVar = tk.StringVar()
        tk.Label(resultFrame, textvariable=self.densityVar).grid(
